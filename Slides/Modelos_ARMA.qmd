...
-   A autocorrelação define um processo MA(q):
    ...
    Seja $Y_t$ um processo MA(q), então a autocorrelação é dada por:

    $$
    \gamma(h)= \begin{cases}\sigma^2 \sum_{j=0}^{q-|h|} \theta_j \theta_{j+|h|}, & \text { if }|h| \leq q, \\ 0, & \text { if }|h|>q,\end{cases}
    $$Isto é, para os lags maiores que a ordem $q$ a autocorrelação é 0.
...
acf = leisure %>%
  ACF(difference(Employed, 12) %>% difference(), lag_max = 48 ) %>% 
  autoplot() + labs(title="ACF")

pacf = leisure %>%
  PACF(difference(Employed, 12) %>% difference(), lag_max = 48) %>% 
