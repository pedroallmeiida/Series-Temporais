...
## <span style="font-size: 40px;"> Função de Autocorrelação </span>

</br>

</br>

**Definição:** Seja $\{ Z_t \}$ um processo estacionário. A função de autocorrelação de $Z_t$ na defasagem (ou lag) h é dada por: 
...
- Para avaliar o grau de dependência nos dados e selecionar um modelo para os dados que reflita isso, uma das ferramentas importantes que usamos é a função de autocorrelação amostral (FCA amostral) dos dados. 
...
- Se acreditarmos que os dados são valores realizados de uma série temporal estacionária $\{Z_t\}$, então a FCA amostral nos fornecerá uma estimativa da FCA de $\{Z_t\}$. 
